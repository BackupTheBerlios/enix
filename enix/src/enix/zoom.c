...
void zoom_setup_image(zoom_image_t *img, int w, int h, int depth, pixel_t *data) {
    img->xsize = h;
    img->ysize = w;
    img->pixspan = depth;
    img->span = w * depth;
...
zoom_image_t *new_image(int xsize, int ysize)  /* create a blank image */
{
    zoom_image_t *image;

    if((image = (zoom_image_t *)malloc(sizeof(zoom_image_t))))
    {

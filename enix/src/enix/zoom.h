...
/* Initializes zoom_image_t structure for given width and height.
   Please do NOT manipulate xsize, span and ysize properties directly,
   zooming code has inverted meaning of x and y axis 
   (i.e. w=ysize, h=xsize)  */
extern void zoom_setup_image(zoom_image_t *img, int w, int h, int depth, pixel_t *data);
...
typedef union
{
   pixel_t      *pixel;   fixdouble     weight;
   int           index;
   int           count;
} instruction_t;
...
extern zoomer_t *zoom_image_init(zoom_image_t *dst, zoom_image_t *src,
